...
public:
    ...
    static constexpr int32_t LastetVersion = 5;
...
public:
    ...
    dVector4(double x = 0, double y = 0, double z = 0, double w = 0) : X(x)
        , Y(y)
        , Z(z)
        , W(w)
    {}
...
public:
    ...
    Curve(const void *data, int32_t size)
    {
        ...
        for (int i = 1; i < mControllPointCount; i++)
        {
            ...
            float len = Vector3D::Length(Vector3D(static_cast<float>(p1.X), static_cast<float>(p1.Y), static_cast<float>(p1.Z)) - Vector3D(static_cast<float>(p0.X), static_cast<float>(p0.Y), static_cast<float>(p0.Z)));
            mLength += len;
        }
    ...
    Vector3D CalcuratePoint(float t, float magnification)
    {
        ...
        float t_rate = static_cast<float>(knot.back() - 1);
        ...
        {
            Vector3D d;
            d.X = static_cast<float>(mControllPoint[j].X) * magnification * static_cast<float>(bs[j]) / static_cast<float>(wSum);
            d.Y = static_cast<float>(mControllPoint[j].Y) * magnification * static_cast<float>(bs[j]) / static_cast<float>(wSum);
            d.Z = static_cast<float>(mControllPoint[j].Z) * magnification * static_cast<float>(bs[j]) / static_cast<float>(wSum);
...
public:
    CurveLoader(FileInterfaceRef fileInterface = nullptr);

    ~CurveLoader() override = default;
