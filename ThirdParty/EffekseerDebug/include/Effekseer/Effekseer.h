...
constexpr int32_t UserUniformSlotMax = 16;

constexpr int32_t UserGradientSlotMax = 2;

//! the maximum number of texture slot including textures system specified
constexpr int32_t TextureSlotMax = 8;

constexpr int32_t LocalFieldSlotMax = 4;

constexpr float EFK_PI = 3.141592653589f;
...
{
    if (t > static_cast<T>(max_))
    {
        t = static_cast<T>(max_);
    }

    if (t < static_cast<T>(min_))
    {
        t = static_cast<T>(min_);
    }
...
inline int32_t ConvertUtf8ToUtf16(char16_t *dst, int32_t dst_size, const char *src)
{
    int32_t i, code = 0;
    int8_t c0, c1, c2 = 0;
    auto srci = reinterpret_cast<int8_t *>(const_cast<char *>(src));
    if (dst_size == 0)
        return 0;
...
template <typename T>
std::shared_ptr<T> CreateReference(T *ptr, bool addRef = false)
{
    if (ptr == nullptr)
...
template <class T>
bool operator==(const RefPtr<T> &lhs, const std::nullptr_t &rhs)
{
    return lhs.Get() == rhs;
}
template <class T>
...
public:
    ...
    bool operator==(const FixedSizeVector<T, N> &rhs) const
    {
        if (size_ != rhs.size_)
            return false;
...
public:
    ...
    ~RenderingUserData() override = default;
...
public:
    StringView() : ptr_(nullptr)
        , size_(0)
    {}

    StringView(const T *ptr) : ptr_(ptr)
        , size_(Traits::length(ptr))
    {}

    StringView(const T *ptr, size_t size) : ptr_(ptr)
        , size_(size)
    {}

    template <size_t N>
    StringView(const T ptr[N]) : ptr_(ptr)
        , size_(N)
    {}

    StringView(const CustomString<T> &str) : ptr_(str.data())
        , size_(str.size())
    {}
...
struct Color
{
    ...
    */
    ...
    /**
      @brief	線形補間
    */
    static Color Lerp(Color in1, Color in2, float t);
...
static constexpr int32_t RenderTargetMax = 4;
...
public:
    UniformLayout(CustomVector<CustomString<char>> textures, CustomVector<UniformLayoutElement> elements) : textures_(std::move(textures))
...
{
    static constexpr int TextureSlotCount = 8;
...
{
    int32_t size = 0;
    if (format == VertexLayoutFormat::R8G8B8A8_UINT || format == VertexLayoutFormat::R8G8B8A8_UNORM)
    {
        size = 4;
    }
    ...
    else if (format == VertexLayoutFormat::R32G32B32_FLOAT)
    {
        size = sizeof(float) * 3;
    }
...
public:
  ...
  */
    ...
    */
    static EffectLoaderRef CreateEffectLoader(FileInterfaceRef fileInterface = nullptr);
...
public:
    ...
    */
    ...
    \~Japanese	描画処理を行う。
    */
    virtual void Draw(const DrawParameter &drawParameter = DrawParameter()) = 0;
    ...
    \~Japanese	背面の描画処理を行う。
    */
    virtual void DrawBack(const DrawParameter &drawParameter = DrawParameter()) = 0;
    ...
    \~Japanese	前面の描画処理を行う。
    */
    virtual void DrawFront(const DrawParameter &drawParameter = DrawParameter()) = 0;
    ...
    */
    virtual void DrawHandle(Handle handle, const DrawParameter &drawParameter = DrawParameter()) = 0;
    ...
    */
    virtual void DrawHandleBack(Handle handle, const DrawParameter &drawParameter = DrawParameter()) = 0;
    ...
    */
    virtual void DrawHandleFront(Handle handle, const DrawParameter &drawParameter = DrawParameter()) = 0;
    ...
    */
    virtual bool GetIsCulled(Handle handle, const DrawParameter &drawParameter) = 0;
...
public:
    ...
    */
    virtual void Update(ManagerRef *managers = nullptr, int32_t managerCount = 0, ReloadingThreadType reloadingThreadType = ReloadingThreadType::Main) = 0;
