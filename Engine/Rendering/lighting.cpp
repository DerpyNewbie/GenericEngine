...
engine::Lighting *engine::Lighting::Instance()
{
    static auto instance = new Lighting;
    return instance;
}

void engine::Lighting::SetCurrentShadowMapIndex(int shadow_map_index)
{
...
void engine::Lighting::UpdateLightsViewProjMatrixBuffer()
{
    int light_vp_idx = 0;
...
void engine::Lighting::SetCascadeSlicesBuffer()
{
...
void engine::Lighting::SetLightsViewProjMatrix() const
{
    const auto cmd_list = RenderEngine::CommandList();
...
void engine::Lighting::SetShadowMap()
{
...
void engine::Lighting::TryApplyShadow(const std::shared_ptr<Light> &light)
{
...
void engine::Lighting::RemoveShadow(const std::shared_ptr<Light> &light)
{
    if (light->m_depth_texture_handle_.empty())
    {
...
void engine::Lighting::AddLight(std::shared_ptr<Light> light)
{
    m_lights_.emplace_back(light);
}

void engine::Lighting::RemoveLight(const std::shared_ptr<Light> &light)
{
...
void engine::Lighting::SetCascadeSlices(
