...
std::shared_ptr<engine::TextureBuffer> engine::TextureCollection::CreateTextureBuffer(const std::shared_ptr<Texture2D>& texture)
{
    if (auto render_texture = std::dynamic_pointer_cast<RenderTexture>(texture))
    {
    ...
    if (auto depth_texture = std::dynamic_pointer_cast<DepthTexture>(texture))
    {
...
std::shared_ptr<engine::TextureBuffer> engine::TextureCollection::GetTexture(const std::shared_ptr<Texture2D>& texture)
{
    ...
    return CreateTextureBuffer(texture);
}

std::shared_ptr<engine::RenderTextureBuffer> engine::TextureCollection::GetRenderTexture(
    const std::shared_ptr<RenderTexture>& render_texture)
{
...
std::shared_ptr<engine::DepthTextureBuffer> engine::TextureCollection::GetDepthTexture(
...
{
    const auto erase_count = m_textures_buffer_map_.erase(render_texture) + m_render_texture_buffer_map_.erase(
        render_texture);
...
{
    const auto erase_count = m_textures_buffer_map_.erase(depth_texture) + m_depth_texture_buffer_map_.erase(
        depth_texture);
