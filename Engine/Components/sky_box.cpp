...
void engine::SkyBox::OnInspectorGui()
{
    ...
    if (changed)
    {
        ...
        if (can_create)
        {
            ...
            {
                m_TextureCubeHandle_ = g_DescriptorHeapManager.Get().Register(m_TextureCube_);
                m_IsTextureSet = true;
            }
        }
    }
...
void engine::SkyBox::OnDraw()
{
    ...
    Matrix view = camera.lock()->GetViewMatrix(); // カメラのView行列（位置含む）
    view.Translation(Vector3::Zero);

    Matrix proj = camera.lock()->GetProjectionMatrix(); // ← 必ず使う
    ...
    auto vbView = m_VertexBuffer_->View();
    auto ibView = m_IndexBuffer_->View();
    cmd_list->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    cmd_list->IASetVertexBuffers(0, 1, &vbView);
    cmd_list->IASetIndexBuffer(&ibView);

    cmd_list->SetPipelineState(g_PSOManager.Get("SkyBox"));
    cmd_list->SetGraphicsRootConstantBufferView(kWVPCBV, viewproj_buffers[current_buffer]->GetAddress());
    cmd_list->SetGraphicsRootDescriptorTable(kPixelSRV, m_TextureCubeHandle_->HandleGPU);

    cmd_list->DrawIndexedInstanced(36, 1, 0, 0, 0);
}

std::weak_ptr<engine::Transform> engine::SkyBox::GetTransform()
{
