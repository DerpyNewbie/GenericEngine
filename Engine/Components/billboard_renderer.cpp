...
void BillboardRenderer::OnConstructed()
{
    ...
    shared_materials[0].CastedLock()->SetShader(AssetPtr<Shader>::FromIAssetPtr(asset_ptr));
...
void BillboardRenderer::Render()
{
    ...
    RenderPipeline::Submit(Primitives::GetQuadMesh(), shared_materials, 1, GameObject()->Transform()->Position(), m_rendering_layer_, m_world_matrix_buffers_[current_buffer_idx]->GetAddress());
}
